...
class ConversationOrchestrator:
    ...
    def _build_messages(self, session: Dict[str, Any], user_message: str) -> List[Dict[str, str]]:
        ...
        preface = (
            ...
            "DB preflight for product names (STRICT): Before saying a product is not found, first attempt normalization and discovery: (1) call search_products with the user's raw wording (translated to English if needed); (2) if no clear match, try category discovery by calling search_products with likely categories (vegetables, fruits, dairy) and offer a short choice; (3) prefer a single clear canonical match and proceed (briefly acknowledge corrections). Never claim 'can't find' without this attempt.\n"
