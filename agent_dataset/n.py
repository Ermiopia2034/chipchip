...
def example_6_predictive_insights():
    """Example 6: Simple predictive insights"""
    print("\n" + "=" * 60)
    print("EXAMPLE 6: Predictive Insights")
    print("=" * 60)
    ...
    daily_demand = df.groupby(['date', 'product_name'])['quantity_ordered'].sum().reset_index()

    # Simple forecast for next 7 days for top products
    top_products = df.groupby('product_name')['order_total_amount'].sum().sort_values(ascending=False).head(3).index

    print(f"📈 Demand Forecast (Next 7 Days)")
    print(f"{'Product':<20} {'Predicted Daily Demand':<25}")
    print("-" * 50)

    for product in top_products:
        product_demand = daily_demand[daily_demand['product_name'] == product]['quantity_ordered']
        if len(product_demand) > 7:
            # Simple 7-day moving average forecast
            recent_avg = product_demand.tail(7).mean()
            print(f"{product:<20} {recent_avg:<25.1f} kg/liters per day")

    # Price optimization insights
    print(f"\n💰 Price Optimization Insights")

    # Find products with high price elasticity (high volume, price-sensitive)
    ...
    high_volume = product_analysis[product_analysis['quantity_ordered'] > product_analysis['quantity_ordered'].quantile(0.7)]

    print(f"High-Volume Products (Consider promotional pricing):")
    for product in high_volume.head().index:
        avg_price = high_volume.loc[product, 'price_per_unit']
        volume = high_volume.loc[product, 'quantity_ordered']
        print(f"  {product}: Avg ETB {avg_price:.2f}, Volume: {volume:.1f} units")
...
def main():
    ...
    try:
        ...
        example_6_predictive_insights()

        print("\n" + "=" * 60)
        print("✅ All examples completed successfully!")
        print("💡 Use these patterns as a starting point for your analysis")
        print("=" * 60)
